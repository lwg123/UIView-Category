...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        flow.itemSize = CGSizeMake(self.width/7, CalendarCellHeight); // 日历一个选项的高度 30
        
        _collectionView =[[UICollectionView alloc]initWithFrame:CGRectMake(0, self.calendarWeekView.bottom + 15, self.width, 6 * CalendarCellHeight) collectionViewLayout:flow];
        _collectionView.delegate = self;
        _collectionView.dataSource = self;
        _collectionView.showsVerticalScrollIndicator = NO;
        _collectionView.showsHorizontalScrollIndicator = NO;
        _collectionView.scrollsToTop = YES;
        _collectionView.backgroundColor = CalendarColor;
...
-(void)setupUI{
    ...
    self.height = self.collectionView.bottom + 60;
...
- (void)backToday {
    // 负数为过去，正数为未来
    if ([self.currentMonthDate daysBetweenCurrentDateAndDate] > 0) {
        self.currentMonthDate = [NSDate date];
        [self performAnimations:kCATransitionFromLeft];
    }else if ([self.currentMonthDate daysBetweenCurrentDateAndDate] < 0) {
        self.currentMonthDate = [NSDate date];
        [self performAnimations:kCATransitionFromRight];
    }else {
        self.currentMonthDate = [NSDate date];
    }
    [self responDataToday];
}
...
- (void)responDataToday {
    ...
    int totalCount;
    if (firstWeekday + monthModel.totalDays > 35) { // 根据天数判断显示五周还是六周
        totalCount = 42;
    }else {
        totalCount = 35;
    }
    ...
    if (totalCount == 42) {
        self.collectionView.height = 6 * CalendarCellHeight;
    }else {
        self.collectionView.height = 5 * CalendarCellHeight;
    }
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.backgroundColor = CalendarColor;
    return cell;
}
...
-(LXCalendarWeekView *)calendarWeekView{
    if (!_calendarWeekView) {
        _calendarWeekView =[[LXCalendarWeekView alloc]initWithFrame:CGRectMake(0, self.calendarHeader.bottom, self.width, 25)];
        _calendarWeekView.weekTitles = @[@"日",@"一",@"二",@"三",@"四",@"五",@"六"];
        _calendarWeekView.backgroundColor = MainColor;
    }
